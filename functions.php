...
load_theme_textdomain( 'whlinenrental', TEMPLATEPATH.'/languages' );
$locale = get_locale();
...
function wp_bootstrap_register_sidebars() {
    ...
    register_sidebar(array(
        'id' => 'people-say-sidebar',
        'name' => 'What People Say',
        'before_title'  => '<ul class="widgettitle">',
        'after_title'   => '</ul>',
    ...
    register_sidebar( array(
        ...
        'name'          => __( 'Contact Us Sidebar', 'text_domain' ),
        'description'   => __( 'Appears in the contact us section of the site.', 'text_domain' ),
...
function theme_admin_styles() {
    ...
    wp_enqueue_style('custom-admin-css', get_template_directory_uri() . '/admin/custom-admin-css.css' );
}
...
function theme_styles() {
    ...
    global $wp_scripts;
    wp_register_script('ie8', get_template_directory_uri() . '/assets/css/ie8.css', '', '', false );
    /* *
     *  Vendor CSS
     * */
    wp_enqueue_style( 'lightSlider', get_template_directory_uri() . '/assets/js/lightslider/lightSlider/css/lightSlider.css' );
    //wp_enqueue_style( 'lightGallery', get_template_directory_uri() . '/assets/js/lightGallery/light-gallery/css/lightGallery.css' );
    wp_enqueue_style( 'mfp-css', get_template_directory_uri() . '/assets/css/magnific-popup.css' );


    /* *
     * Theme CSS
     * */
    wp_enqueue_style( 'boots', get_template_directory_uri() . '/assets/css/bootstrap.css');
    wp_enqueue_style( 'theme_styles', get_template_directory_uri() . '/assets/css/styles.css' );
    wp_enqueue_style( 'theme_styles_ie', get_template_directory_uri() . '/style.css' );
    
    wp_enqueue_style( 'theme_styles_ie', get_template_directory_uri() . '/assets/css/ie.css' );
    $wp_scripts->add_data( 'ie8', 'conditional', 'lt IE 8' );
...
function theme_js() {
    ...
    wp_enqueue_script( 'placeholder_js', get_template_directory_uri() . '/assets/js/jquery-placeholder/jquery.placeholder.min.js', array('jquery'), '', true );

    wp_enqueue_script( 'mixitup', get_template_directory_uri() . '/assets/js/mixitup-master/build/jquery.mixitup.min.js', array('jquery'), '', true );
    wp_enqueue_script( 'lightSlider', get_template_directory_uri() . '/assets/js/lightslider/lightSlider/js/jquery.lightSlider.min.js', array('jquery'), '', true );
    //wp_enqueue_script( 'lightGallery', get_template_directory_uri() . '/assets/js/lightGallery/light-gallery/js/lightGallery.min.js', array('jquery'), '', true );
    wp_enqueue_script( 'mfp-js', get_template_directory_uri() . '/assets/js/magnific-popup/magnific-popup.min.js', array('jquery'), '', true );


    /*Must be called last*/
    wp_enqueue_script( 'site_scripts', get_template_directory_uri() . '/assets/js/scripts.js', array('jquery'), '', true );
...
function wp_bootstrap_custom_admin_footer() {
    echo '<span id="footer-thankyou">Developed by <a href="http://creatingdigital.com" target="_blank">Creating Digital</a></span>.';
}
...
/*************************
 *  **********************
 * WHAT CLIENTS SAY SIDEBAR & Widget
 * Adds clients_quotes__widget widget.
 */
class clients_quotes__widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'clients_quotes__widget', // Base ID
            __( 'Clients" Quotes Widget ', 'text_domain' ), // Name
            array( 'description' => __( 'Used in clients quotes sidebar', 'text_domain' ), ) // Args
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        if ( ! empty( $instance['client_quote'] ) ) {
            echo "<div class=\"quote-slide\">";
            echo "<span class='quote'>" . apply_filters( 'widget_title', $instance['client_quote'] ). "</span>";
            echo "</div>";
        }
    ...
    public function form( $instance ) {
        $client_quote = ! empty( $instance['client_quote'] ) ? $instance['client_quote'] : __( 'New quotation', 'text_domain' );
        ?>
        <div>
            <label for="<?php echo $this->get_field_id( 'client_quote' ); ?>"><?php _e( 'Quote: Add quote' ); ?></label>
            <textarea class="widefat" id="<?php echo $this->get_field_id( 'client_quote' ); ?>" name="<?php echo $this->get_field_name( 'client_quote' ); ?>"
            value="<?php echo esc_attr( $client_quote ); ?>" placeholder="Type client's quote" style="width: 100%"><?php echo esc_attr( $client_quote ); ?></textarea>
        </div>
    ...
    public function update( $new_instance, $old_instance ) {
        $instance = array();
        $instance['client_quote'] = ( ! empty( $new_instance['client_quote'] ) ) ? strip_tags( $new_instance['client_quote'] ) : '';

        return $instance;
    }

} // class clients_quotes__widget
// register clients_quotes__widget widget
function register_clients_quotes__widget() {
    register_widget( 'clients_quotes__widget' );
}
add_action( 'widgets_init', 'register_clients_quotes__widget' );
